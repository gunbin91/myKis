...
- **Source**: `(해외주식) 실시간시세.xlsx` / sheet `해외주식 실시간호가`
- **API ID**: `실시간-021`
- **통신방식**: **WEBSOCKET**
- **HTTP Method**: `POST`
...
| Response Header |  |  |  |  |  |  |
| Response Body | RSYM | 실시간종목코드 | object | Y | 16 | '각 항목사이에는 구분자로 ^ 사용, 모든 데이터타입은 String으로 변환되어 push 처리됨' |
